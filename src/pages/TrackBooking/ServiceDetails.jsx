...
const ServiceDetails = ({ booking }) => {
  ...
  const estimatedCompletion = '12:15 PM'; // You can calculate this dynamically if needed
  ...
  return (
    <div className='bg-white rounded-xl shadow p-6'>
      ...
      <div className='grid grid-cols-2 gap-y-2 text-sm mb-4'>
        ...
        <p className='text-gray-600'>Urgent Booking</p>
        <p className='font-medium'>
          {booking.booking_is_urgent ? 'Yes' : 'No'}
        </p>

        <p className='text-gray-600'>Status</p>
        <p className='font-medium capitalize'>{booking.booking_status}</p>
      </div>

      <div className='flex items-start gap-2 mb-4'>
        <MapPin className='text-gray-500 w-4 h-4 mt-1' />
        <p className='text-sm text-gray-800'>
          {booking.customer_address}, {booking.customer_city} -{' '}
      ...
      <div>
        <p className='text-gray-600 mb-1 text-sm'>Assigned Worker</p>
        <p className='bg-gray-100 text-sm text-gray-800 p-2 rounded'>
          {booking.worker_name} ({booking.worker_skills.join(', ')}) — ₹
          {booking.worker_hour_rate}/hr
        </p>
      </div>
    </div>
  );
