  ...
  return (
    <div className="p-2 ">
      {jobs.map((job) => (
        <div
          key={job._id}
          className="bg-white shadow rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between border"
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 mt-4 md:mt-0">
            ...
            {job.booking_status === "in-progress" ? (
              <button className="px-4 py-2 rounded-lg bg-green-600 text-white font-medium hover:bg-green-700" onClick={() => handleComplete(job._id)}>
                Mark Complete
              </button>
            ) : (
              <button className="px-4 py-2 rounded-lg bg-black text-white font-medium hover:bg-gray-800" onClick={() => handleInProgress(job._id)}>
                Start Job
              </button>
            )}
          </div>
